...
def shortest_string(strings):
    ...
    # ------------------------------------------------------------------
    k_current_shortest = 0
    for k in range(1, len(strings)):
        if len(strings[k]) < len(strings[k_current_shortest]):
            k_current_shortest = k
    return strings[k_current_shortest]
...
def index_of_largest_number(numbers, n):
    ...
    k_for_largest = 0
    for k in range(1, n):
        if numbers[k] > numbers[k_for_largest]:
            k_for_largest = k
    return k_for_largest
